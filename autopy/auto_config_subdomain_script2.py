...
def create_apache_config():
    config = """
<VirtualHost *:80>
    ServerName api1.tospos.my.id

    ProxyPreserveHost On
    ProxyPass / http://35.209.213.218:3002/
    ProxyPassReverse / http://35.209.213.218:3002/

    ErrorLog ${APACHE_LOG_DIR}/api1_error.log
    CustomLog ${APACHE_LOG_DIR}/api1_access.log combined
</VirtualHost>
"""
    with open('/etc/apache2/sites-available/api1.tospos.my.id.conf', 'w') as f:
        f.write(config)
    print("Apache configuration file created for api1.tospos.my.id")

def enable_site_and_reload():
    run_command("a2ensite api1.tospos.my.id.conf")
    run_command("systemctl reload apache2")

def obtain_ssl_certificate():
    print("Obtaining SSL certificate for domain: api1.tospos.my.id...")
    run_command("certbot --apache -d api1.tospos.my.id --non-interactive --agree-tos --email your-email@example.com")
    print("SSL certificate obtained for api1.tospos.my.id")
...
def main():
    ...
    run_command("systemctl reload apache2")
    print("Configuration for api1.tospos.my.id has been successfully applied.")
